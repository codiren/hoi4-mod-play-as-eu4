...
int main()
{
	...
	while(std::getline(ine,t)){
		if(kitascores){
			if(t.find("}")!=-1){kitascores = false;terainas = "plains";std::cout<<5;continue;}
			if(t.find("#")!=18446744073709551615)t = split(t,"#")[0];
			tv = split(t," ");
			for(std::string kas:tv){
			//std::cout<<<<endl;
				if(std::atoi(kas.c_str())==153)std::cout<<terainas<<"\n";
...
//	outrocketsites.close();
    ...
    std::ifstream snar("map/definition - old.csv");
    std::ifstream snark("map/definition.csv");
    std::getline(snar,t);
    bool kelin[5000];
    std::string full = "0;0;0;0;land;true;unknown;0\n";
    std::getline(snark,t);
    std::vector<std::string> tvaea;
    while(std::getline(snar,t)){
    	tv = split(t,";");
    	std::getline(snark,t);
    	tvaea = split(t,";");
    	full += tv[0]+";"+tv[1]+";"+tv[2]+";"+tv[3]+";"+tv[4]+";"+tv[5]+";"+tvaea[6]+";"+tv[7]+"\n";
	}
    snar.close();
    snark.close();
    std::ofstream snarka("map/definition.csv");
    snarka<<full;
    snarka.close();
